...
tensao_alimentacao = 5 #Defina a tensão da bancada com a qual esta trabalhando

numAmostras = 600  #Numero de amostras que serão coletadas
tempo = np.zeros(numAmostras) #Arrays para armazenar o tempo e as leituras de tensão
y = np.zeros(numAmostras)

Ts = 1000 #20e-3 #Tempo de amostragem (intervalo de tempo entre as amostras)

fre = 2 #Frequencia do sinal gerado
Amplitude = 0.0 #Amplitude do sinal gerado
setpoint = 2.5 #valor desejado para o controle
...
conexao = serial.Serial(port='COM9', baudrate=9600, timeout=0.005) #Alterar a porta de conexão serial de acordo com a do arduino
